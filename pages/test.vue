<template>
  <div class="flex justify-center items-center h-screen">
    ...
    >
      ...
      >
        Open Delete Modal (replace with trash can icons)
      </button>
      <modals-action-delete
        v-if="showDeleteModal"
        @confirm-action="deleteModalConfirmAction"
      ...
      >
        ...
        <p class="mt-6 font-bold">Are you sure you want to delete this item?</p>
      </modals-action-delete>
      <!-- ACTION RESTORE MODAL -->
      <button
        @click="restoreModalControl"
        class="inline-block rounded bg-blue-500 hover:bg-blue-700 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white"
      >
        Open Restore Modal (replace with icon or button)
      </button>
      <modals-action-restore
        v-if="showRestoreModal"
        @confirm-action="restoreModalConfirmAction"
      ...
      <button
        @click="accountSuccessfulModalControl"
        class="inline-block rounded bg-blue-500 hover:bg-blue-700 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white"
      >
        Open Restore Modal (should be controlled by an async function that
        triggers after account creation)
      </button>
      <modals-account-successful
        v-if="showAccountSuccessful"
        @confirm-action="accountSuccessfulModalConfirmAction"
      >
        <div class="flex justify-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <p class="mt-6 font-bold">Account Created Successfully</p>
      </modals-account-successful>
      <!-- STATUS CHANGE MODAL -->
      <button
        @click="statusChangeModalControl"
        class="inline-block rounded bg-blue-500 hover:bg-blue-700 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white"
      >
        Open Status Change Modal (should be controlled by an async function that
        triggers after status change)
      </button>
      <modals-account-successful
        v-if="showStatusChange"
        @confirm-action="statusChangeModalConfirmAction"
      >
        <div class="flex justify-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <p class="mt-6 font-bold">Account status changed successfully</p>
      </modals-account-successful>
    </div>
  </div>
</template>
