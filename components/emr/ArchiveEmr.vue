<template>
  <div
    ...
    id="archiveUser"
    tabindex="-1"
    aria-labelledby="archiveUserLabel"
    aria-hidden="true"
    role="dialog"
  >
    <div
      ...
      <div
        ...
        <div class="relative flex-auto p-4 text-center" data-te-modal-body-ref>
          Are you sure you want to disable selected responder account/s?
        </div>
      ...
      if (this.ids.length > 0) {
        ...
        fetch('https://ipinfo.io/json?token=5d9e0b426ac4f6')
          ...
          .then((response) => {
            ...
            this.$axios.get('user/archive?ids=' + this.ids + '&user_name=' + this.$auth.user.email + '&user_role=' + this.$auth.user.role + '&ip_address=' + this.id_address).then(() => {
              location.reload();
            });
          })
